...
grades = (81, 89, 94, 78, 61, 66, 99, 74)  #  tuple

# TODO: use map to create a new sequence of values

squared_nums = list(map(squareFunc, nums))
print(squared_nums)

# TODO: use sorted and map to change numbers to grades
grades = sorted(grades)
grade_chars = list(map(toGrade, grades))
print(grade_chars)
...
def simplify(q):
    return {"date": str(datetime.date.fromtimestamp(q["properties"]["time"]/1000)),
            "place": q["properties"]["place"],
            "magnintude": q["properties"]["mag"]
            }
